...
using MySample.WebAPI.Models;
using MySample.WebAPI.Providers;
using MySample.WebAPI.Results;
using MySample.Models;
using MySample.Data;
using System.Diagnostics;
using System.Threading;

namespace MySample.WebAPI.Controllers
{
    [Authorize]
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<ManageInfoViewModel> ManageInfo(string returnUrl, bool generateState = false)
        {
            ...
            UserPropertiesViewModel userProperties = UserPropertiesList();
        ...
        // GET api/Account/UserProperties
        [HttpGet("UserProperties")]
        public UserPropertiesViewModel UserPropertiesList()
        {
            ...
            return Task.Run(async () =>
            {
                using (CustomDbContext db = new CustomDbContext())
                {
                    try
                    {
                        Store<CustomUser> userstore = new Store<CustomUser>(db);
                        var user = await userstore.GetByIdAsync(User.Identity.GetUserId(), CancellationToken.None);
        ...
        // POST /api/Account/UserProperties
        [HttpPost("UserProperties")]
        public async Task<IHttpActionResult> UpdateUserProperties(UserPropertiesViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
