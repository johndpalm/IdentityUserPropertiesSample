function ManageViewModel(app, dataModel) {
    ...
    self.updateUserProperties = ko.computed(function () {
        return new UpdateUserPropertiesViewModel(app, self, dataModel);
    });
            ...
            dataModel.getManageInfo(dataModel.returnUrl, true /* generateState */)
                .done(function (data) {
                    if (typeof (data.localLoginProvider) !== "undefined" &&
                        ...
                        typeof (data.userProperties) !== "undefined") {
                        ...
                        self.firstName(data.userProperties.firstName);
                        self.lastName(data.userProperties.lastName);
                        self.email(data.userProperties.email);
                        self.phone(data.userProperties.phone);
                    } else {
...
function UpdateUserPropertiesViewModel(app, parent, dataModel) {
    var self = this;

    self.firstName = ko.observable(parent.firstName());
    self.lastName = ko.observable(parent.lastName());
    self.email = ko.observable(parent.email());
    self.phone = ko.observable(parent.phone());
    ...
    self.update = function () {
        self.errors.removeAll();
        self.updating(true);
        dataModel.updateUserProperties({
            ...
            self.updating(false);
            parent.message("User properties updated.");
        }).failJSON(function (data) {
            var errors;
