...
namespace MySample.WebMVC.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        {
            IdentityManager = new AuthenticationIdentityManager(new IdentityStore(new CustomDbContext()));
        }
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(RegisterExternalViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                IdentityResult result = await IdentityManager.Authentication.CreateAndSignInExternalUserAsync(AuthenticationManager, new CustomUser(model.UserName));
                if (result.Success)
                {
                    return RedirectToLocal(returnUrl);
                }
